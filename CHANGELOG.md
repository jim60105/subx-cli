...
- Installer libc selection (`SUBX_LIBC` env var, `--musl` flag, best-effort `ldd` auto-detection) and exact-match URL resolution that no longer confuses `subx-linux-x86_64` with `subx-linux-x86_64-musl`.
- Improved installer diagnostics: when the expected asset is missing, the installer now prints the detected platform/arch, searched asset name, list of available assets, and a link to the releases page.
- Local / OpenAI-compatible LLM provider (`ai.provider = "local"`, alias `ollama` normalized to `local` at config write time). Supports Ollama, LM Studio, llama.cpp `llama-server`, vLLM, and any OpenAI-compatible HTTP endpoint reachable over loopback, LAN, VPN/tailnet, or the public internet. `ai.api_key` is optional and `ai.base_url` is required for `local`; both `http://` and `https://` schemes are accepted. The new `LocalLLMClient` reuses the shared `PromptBuilder`, `ResponseParser`, and `HttpRetryClient` traits and is wired through `ComponentFactory::create_ai_provider`. New env vars `LOCAL_LLM_BASE_URL` and `LOCAL_LLM_API_KEY` are honored only when the canonical provider is `local`. Privacy carve-out: when `local` is selected, hosted-provider env vars (`OPENAI_API_KEY`, `OPENAI_BASE_URL`, `OPENROUTER_API_KEY`, all `AZURE_OPENAI_*`) are skipped entirely so they cannot silently switch the active provider or inject credentials. Hosted providers (`openai`, `openrouter`, `azure-openai`) now require `https://` for `ai.base_url`; misconfigured local endpoints surface a one-line advisory hint that points to `ai.provider = "local"` (or `ollama`).
- Machine-readable JSON output mode for scripting and automation. The new top-level `--output <text|json>` flag (must precede the subcommand token) and the `SUBX_OUTPUT` environment variable opt every covered subcommand into a stable, versioned JSON envelope on stdout. The envelope ships at `schema_version: "1.0"` with `command`, `status`, `data` (on success), and `error` (on failure). Covered subcommands: `match`, `sync`, `convert`, `detect-encoding`, `cache` (`status`/`clear`/`rollback`/`apply`), `translate`, and `config`. Per-file batch commands carry per-item `status`/`error` so partial failures keep the top-level envelope `ok`. `generate-completion` explicitly rejects JSON mode with `error.code == "E_OUTPUT_MODE_UNSUPPORTED"`. The legacy `cache status --json` flag is preserved as a byte-identical alias for the global flag. Existing exit codes, default text UX, and progress bars are unchanged in text mode. See [`docs/machine-readable-output.md`](docs/machine-readable-output.md) for the full envelope schema, error categories, per-command payload schemas, and scripting recipes.
- Top-level `--quiet` flag (also positioned before the subcommand) that suppresses status chatter in text mode and additionally silences free-form stderr diagnostics in JSON mode while leaving the stdout envelope intact.
- Stable error-envelope mapping with `category()` and `machine_code()` helpers on every `SubXError` variant, plus a synthetic `argument_parsing` / `E_ARGUMENT_PARSING` envelope for clap argument-parsing failures so JSON-mode invocations always emit exactly one JSON document on stdout (including before any subcommand runs).
