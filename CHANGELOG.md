...
### Removed
- Linux musl release artifacts (`subx-linux-x86_64-musl`, `subx-linux-aarch64-musl`) added in v1.7.0 are no longer published. The upstream ONNX Runtime distribution consumed by SubX-CLI's voice-activity-detection support does not ship precompiled musl binaries (`ort` 2.0.0-rc.10's `dist.txt` has no `*-unknown-linux-musl` entries), and source-building it for musl is out of scope for this release pipeline. The release matrix and `Cross.toml` now cover only `aarch64-unknown-linux-gnu` for cross-compilation; the README install table and zh-TW counterpart drop the musl rows. `scripts/install.sh` still parses `--musl` and `SUBX_LIBC=musl` but exits non-zero (exit code 2) — and now also rejects when `--musl` is set even alongside `SUBX_LIBC=gnu`, so a conflicting selector cannot silently fall through. Auto-detected musl hosts (`ldd --version` reports musl) get the same exit. The error message and both READMEs document the supported migration path: provision a musl-compatible ONNX Runtime locally, export `ORT_LIB_LOCATION` to point at it, then run `cargo install subx-cli`. Plain `cargo install subx-cli` (without `ORT_LIB_LOCATION`) is *not* a working alternative because `ort`'s default `download-binaries` feature shares the same upstream gap. `compute_binary_name` and `select_download_url` continue to handle `-musl` asset names so the helper-function unit tests in `scripts/test_install.sh` still pass unchanged. The harness gains seven new cases covering the rejection contract (`SUBX_LIBC=musl`, `--musl`, and the `SUBX_LIBC=gnu --musl` precedence case).
