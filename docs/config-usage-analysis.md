...
| `model` | String | "gpt-4o-mini" | **呼叫樹:**<br>• `MatchCommand::execute()` (line 173) → `src/commands/match_command.rs:173`<br>• `OpenAIClient::from_config()` (line 224) → `src/services/ai/openai.rs:224`<br>• `OpenAIClient::chat_completion()` 使用模型進行 HTTP 請求 | 指定使用的 OpenAI 模型，在 HTTP 請求中使用 | `subx-cli match` | ✅ 使用中 |
